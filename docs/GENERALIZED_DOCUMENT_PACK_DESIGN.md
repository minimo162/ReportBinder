...
- 状態: 段階実装中（PR 5相当のschema 3・共通UI/API・Excel/Word/PDF原稿アダプターまで実装済み）
- 作成日: 2026-08-06
- 対象: 現行 Local API V4 / structure schemaVersion 2 からの段階移行
...
PowerPoint、画像、メール、OCR、Office 文書の意味的な赤入れは初期範囲に含めない。まず、異種原稿を混在させても現在の更新検知・履歴・構成・最終出力が一貫して動くことを完成条件とする。
...
5. **最終出力**: 出力先別の準備状況、ブロッカー、作成、公開先へのコピー
...
- `.docm`, `.xlsm`, PowerPoint、画像、メールの取り込み
...
PR 1〜6は実装済みである。schema 3とV4互換viewを維持したまま、Excel・Word・PDFを同じ原稿一覧、更新検出、物理ページ構成、履歴差分へ接続した。Wordは専用COMプロセス、一時コピー、マクロ・リンク・フィールド更新抑止、120秒timeout、所有プロセスだけの終了を実装している。
...
PR 7まで実装済みである。itemからのPDFブックマーク、任意targetの出力、pageRangeによるWord/PDF部分利用、manifestの物理ページ追跡まで組版を一般化した。PR 8では配布、移行、復旧、実機E2Eを完成させる。
