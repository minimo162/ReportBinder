...
PR 1〜8は実装済みである。schema 3とV4互換viewを維持したまま、Excel・Word・PDFを同じ原稿一覧、更新検出、物理ページ構成、履歴差分、最終組版、配布・復旧へ接続した。Wordは専用COMプロセス、一時コピー、マクロ・リンク・フィールド更新抑止、120秒timeout、所有プロセスだけの終了を実装している。
...
PR 7でitemからのPDFブックマーク、任意targetの出力、pageRangeによるWord/PDF部分利用、manifestの物理ページ追跡まで組版を一般化した。PR 8で配布、移行、復旧、実機E2E、大容量・混在原稿の性能受け入れを完成させ、汎用資料パックMVPの完成条件を満たした。次段階では、リリース品質ゲートを自動化した後、ECM / BOD / DMM固定表示を任意資料パック作成UIへ拡張する。
