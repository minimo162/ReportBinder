...
部署別のExcel・Word・PowerPoint・PDF原稿をページ単位のPDFへ変換し、必要な順番で1つ以上の提出用PDFへまとめるWindows向けツールです。

V5では、用途ごとの「資料パック」を管理単位に、提出用PDFの鮮度判定、ページ順の復旧、複数PCからの同時更新、UI導線を全面的に改修しています。

Excel / Word / PowerPoint / PDF を混在できる汎用資料パックへの拡張方針は、[汎用資料パック化 設計書](docs/GENERALIZED_DOCUMENT_PACK_DESIGN.md)を参照してください。
...
資料パックが唯一の管理単位です。日本語・英語などの用途は資料パック名やひな形で分け、アプリや保存領域は分割しません。管理データは `%LOCALAPPDATA%\ReportBinder\projects\<提出フォルダー識別子>\data\workspace` に保存します。旧 `data\ja` / `data\en` の資料は読み込みません。
...
導入、既存案件の移行、障害復旧、容量見積もりは [配布・移行・運用ガイド](docs/OPERATIONS_GUIDE.md) を参照してください。提出フォルダー画面では、Excel・Word・PDF処理と保存先を確認する動作環境診断を実行できます。

### 配布物の完全性検証

配布物の作成時に、`app` 配下の全ファイルのSHA-256とサイズを `app\integrity-manifest.json` へ記録します。起動時、共有フォルダーから `%LOCALAPPDATA%` へコピーしたツリーを確定する前に全ファイルを照合し、内容の相違・サイズの相違・ファイルの欠落・記録にない余分なファイルのいずれかを見つけた場合は、起動を中止して理由を表示します。共有コピーでの続行は行いません。記録は `%LOCALAPPDATA%\ReportBinder\logs\integrity-latest.log` に残ります。

これが防ぐのは、SMB越しのコピー欠落・切り詰めと、配布ツリーの一部だけを書き換えられた場合です。**共有フォルダー全体に書き込める相手は、この検証を行う `launch.ps1` とマニフェスト自体を同時に書き換えられるため、検証を回避できます。** 配布用の共有フォルダーは、発行者以外に対して読み取り専用に設定してください。それより強い保護が必要な場合は、配布する `.ps1` にコード署名を行い、署名の検証を追加してください。

マニフェストのない配布物（旧版、開発ツリーからの直接起動）では検証を省略し、警告のみを記録します。
...
提出原稿の履歴、比較用PDF、ページ構成は利用者ごとのローカルプロジェクトに保持します。
同じ提出フォルダを複数人が選んでも、登録状態・比較基準・ページ構成・出力は互いに影響しません。
保持世代数、容量上限、自動処理の設定は `docs\INPUT_HISTORY_POLICY.md` を参照してください。

登録・PDF化画面では、最新のPDF作成結果と見た目の変更件数だけを簡潔に表示します。
「詳細を見る」または左ナビの「変更履歴・比較」から、任意の2世代を選んで差分詳細を開けます。前回版と現在版の左右比較、同期スクロール・倍率、
...
Excelはシート名、Word/PDFは内容が一致するページ列を基準に比較元と比較先を対応付けます。
途中へのページ挿入後も後続ページをまとめて変更扱いにせず、対応の確信度を画面に表示します。
追加・削除・変更が重なり対応を確定できない区間は「判定不能」とし、左右PDFの目視確認を促します。
...
共有発行: 提出フォルダ\MMdd_HHmmss_資料パック名_Windowsユーザー名
```
...
左ナビの項目に沿って進めます。原稿フォルダの選択は最初の「原稿を登録・PDF化」に含まれ、作業中もナビの位置は変わりません。
...
資料パックは登録済み原稿、ページ構成、提出用PDFの状態・ファイル・manifestまで互いに分離されます。初回は資料パック0件から始まり、用途が分かる名前を付けて作成します。

### 原稿登録・変換PDF作成

1. 未登録のExcel、Word、PowerPointまたはPDFを選び、登録ボタンを押します。
2. 登録直後は登録した原稿が選択済みになります。
3. `PDF作成`を押します。未選択の場合は変換PDF作成が必要な原稿だけを処理します。
...
登録対象は提出フォルダ直下の`.xlsx`、`.docx`、`.pptx`、`.pdf`です。Word、PowerPoint、PDFは物理ページごとに分割してページ構成へ追加し、差し替え時にはページ追加・削除と見た目の差分を履歴へ反映します。パスワード保護・暗号化・破損ファイルは登録時に拒否し、マクロを含む原稿は受け付けません。子フォルダ、`_reportbinder`、`出力`は対象にしません。

Word原稿は提出元を直接開かず、読み取り専用の一時`.docx`コピーを専用の非表示WordプロセスでPDF化します。リンク・フィールド更新、マクロ実行、変更履歴の印刷を無効にし、120秒でタイムアウトします。変換終了時は専用Wordプロセスだけを終了し、利用者が開いているWordには触れません。

### ページ構成

Excelの表示中ワークシートは、シート名が日本語・英数字・記号のどれでも登録できます。
新しく見つかったページは、まず `未振り分け（出力しない）` に入ります。必要なページだけを
`本体` または `補足` へ移してください。明示的に振り分けたページだけが提出用PDFに含まれます。

```text
未振り分け（出力しない） → 本体 / 補足 → 提出用PDF
```

- `Alt+↑/↓`で1行移動
- `Alt+Shift+↑/↓`で先頭・末尾へ移動
- 行またはファイル名をダブルクリック／クリックしてプレビュー
- `原稿内の順序に整える`で、各表の割り当てを変えずに原稿内の順序へ戻す
...
| 状態 | 表示 |
|---|---|
| 出力条件を満たさない | 提出用PDFを出力できません |
| 出力後に入力が変わった | 提出用PDFの再出力が必要 |
| 記録はあるがファイルがない | 前回出力が見つかりません |
| 初回出力前 | 提出用PDFは未出力 |
| fingerprint一致・ファイル実在 | 提出用PDFは最新です |

前回PDFが実在する場合、再出力が必要な状態でも`前回出力を開く`を利用できます。最新の状態では`出力したPDFを開く`になり、`保存先フォルダーを開く`でエクスプローラーに選択状態で開けます。
通常の提出用PDFはローカルへ作成します。`共有発行`を押すと、提出フォルダ直下へ
`MMdd_HHmmss_資料パック名_Windowsユーザー名` の発行フォルダを作ります。PDFのファイル名は
ローカル提出用PDFと同じままです。同じ秒に同じ利用者が発行した場合は、フォルダ名へ連番を付けて既存フォルダを上書きしません。

## 鮮度判定

提出用PDFを作成したときの`builtFingerprint`と現在の入力fingerprintを比較します。fingerprintには次を含めます。
...
元Excelの`currentExcelHash`は組版fingerprintに含めません。組版中に元Excelだけが更新された場合、完成したPDFは保持しつつ、完了直後に`PDF作成が必要`および`提出用PDFの再出力が必要`へ遷移します。

カテゴリを省略した提出用PDF系APIはHTTP 400で拒否します。空categoryを全カテゴリ一致として扱うfail-open経路はありません。
...
提出用PDFは組版開始時と完了時にfingerprintを再比較します。ページ順またはcontent-pdfが組版中に変わった場合、一時PDFを破棄し、既存の提出用PDFを差し替えません。
...
提出用PDFでは奇数ページを右へ、偶数ページを左へ0.2cm移動します。ページ番号はArial 8pt、`- 2 -`形式です。番号非表示ページも通番には含めます。

## 診断・停止

```text
app\tools\diagnostic.cmd
...
提出用PDFの表紙・目次・区切り・ページ範囲・ブックマークを実PDFで確認する場合:

```powershell
python app\tools\final-composition-selfcheck.py
```

「提出用PDF」の「資料の仕上げ」では、出力ファイル名パターンを資料パック別に設定できます。表紙・目次・区切りページも通常の原稿として登録し、ページ構成で配置します。ページ構成の各カードでは、変換PDFから使用するページを`2-5`のように指定できます。出力後のmanifestには物理ページ番号と、各ページの元原稿・元PDFページが保存されます。
