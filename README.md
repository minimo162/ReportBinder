...
部署別のExcel・Word・PDF原稿を、シートまたは物理ページ単位のcontent-pdfへ変換し、本体・補足の最終PDFへ組版するWindows向けツールです。

V4では、最終PDFの鮮度判定、ECM/BOD/DMMの分離、ページ順の復旧、複数PCからの同時更新、UI導線を全面的に改修しています。

Excel / Word / PDF を混在できる汎用資料パックへの段階的な拡張方針は、[汎用資料パック化 設計書](docs/GENERALIZED_DOCUMENT_PACK_DESIGN.md)を参照してください。
...
登録済み原稿の「変更 N項目」「追加 N項目」「削除 N項目」「変更なし」
...
カテゴリは登録済み原稿、ページ構成、最終PDFの状態・ファイル・manifestまで完全に分離されます。

### 原稿登録・変換PDF作成

1. 未登録のExcel、WordまたはPDFを選び、登録ボタンを押します。
...
登録対象は提出フォルダ直下の`.xlsx`、`.docx`、`.pdf`です。WordとPDFは物理ページごとに分割してページ構成へ追加し、差し替え時にはページ追加・削除と見た目の差分を履歴へ反映します。パスワード保護・暗号化・破損ファイルは登録時に拒否し、`.docm`やマクロを含むWord原稿は受け付けません。子フォルダ、`_reportbinder`、`出力`は対象にしません。
...
同じ完全検査は、すべてのpull requestとmainへのpushで`.github/workflows/ci.yml`から自動実行されます。Excel / Word COMを使う実機E2Eとlargeベンチマークは、対話Windowsセッションが必要なためリリース受け入れ時に別途実行します。
...
「提出用PDF」の「資料の仕上げ」では、資料タイトル、サブタイトル、表紙、目次、原稿ごとの区切り、出力ファイル名パターンを資料パック別に設定できます。ページ構成の各カードでは、変換PDFから使用するページを`2-5`のように指定できます。出力後のmanifestには生成ページを含む物理ページ番号と、各ページの元原稿・元PDFページが保存されます。
