...
V5では、用途ごとの「資料パック」を管理単位に、最終PDFの鮮度判定、ページ順の復旧、複数PCからの同時更新、UI導線を全面的に改修しています。
...
左ナビの3ステップに沿って進めます。原稿フォルダの選択は最初の「原稿を登録・PDF化」に含まれ、作業中もナビの位置は変わりません。
...
資料パックは登録済み原稿、ページ構成、最終PDFの状態・ファイル・manifestまで互いに分離されます。初回は資料パック0件から始まり、用途が分かる名前を付けて作成します。
