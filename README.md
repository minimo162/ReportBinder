...
変更を加える前に [docs/DEVELOPMENT_RULES.md](docs/DEVELOPMENT_RULES.md) を読んでください。とくに次の2点は、このリポジトリで繰り返し事故が起きた箇所です。

- **新しい仕組みを足したら、それを疑う工程を必ず挟む。** 監視・上限・再試行といった「安全のための仕組み」が、それ自体で正常動作を壊した例が複数あります（正常な重いExcel変換が強制終了された、健全なPDFが「破損」と診断された、など）。いずれも実機テストは通っていました。
- **追加した検査は、わざと壊して落ちることを確かめる。** 空振りしていた検査が繰り返し見つかっています。

対応しないと決めたこと（サブフォルダー探索、旧形式の登録、`workbookId` のリネーム）も同じ文書にまとめてあります。
