...
let activePreset = '';
// 一式の選択はタブを越えて覚える。起動は毎回「新しいタブ」なので sessionStorage
// だけだと空になり、既定が「作成順の先頭」＝最も古い一式になっていた。1か月ぶりに
// 開いた人が、先月以前の一式に今月の原稿を登録し、提出用PDFを開くまで気づかない。
let activePackId = (() => {
  try { return sessionStorage.getItem('ReportBinderPackId') || localStorage.getItem('ReportBinderPackId') || ''; } catch { return ''; }
})();
function rememberActivePack(packId) {
  try {
    if (packId) { sessionStorage.setItem('ReportBinderPackId', packId); localStorage.setItem('ReportBinderPackId', packId); }
    else { sessionStorage.removeItem('ReportBinderPackId'); localStorage.removeItem('ReportBinderPackId'); }
  } catch {}
}
let packAdminPacks = [];
...
// Start with the whole composition in view. A page organizer is primarily a
// spatial task: hiding two destinations behind tabs makes users memorise where
// pages went and removes the most useful drop targets. Tabs remain available as
// a focus mode for large packs and narrow screens.
let activePageVolume = (() => { try { return sessionStorage.getItem('ReportBinderActivePageVolume') || 'all'; } catch { return 'all'; } })();
...
// 同じタブでの移動は覚える。新しいタブ（＝.cmd からの起動）は「ホーム」に着地させる。
// 以前は毎回ステップ1に降りていたため、1か月ぶりに開いた人が「前回どこまで
// 終わったか」を示すカードに一度もたどり着けなかった。
let activeView = (() => {
  try {
    const stored=sessionStorage.getItem('ReportBinderView');
    if(!stored) return 'dashboard';
    return stored==='folders'?'excel':stored;
...
function setActiveView(view, options = {}) {
  ...
  document.querySelector('main.shell')?.classList.toggle('pages-shell', activeView === 'pages');
...
// 出力先チップは色だけで3つの警告状態を分けていた。加齢による色の見分けにくさと
// 正面衝突するので、状態を語で添える。
function targetStateSuffix(displayState){
  const map={'needs-rebuild':'・要再出力','output-missing':'・ファイルなし','not-built':'・未出力'};
  return map[String(displayState||'')]||'';
}
...
function isLatestPdfWorkbook(w) {
  ...
  if (sourceTypeValue(w) === 'excel') {
    const wantedSheetSelection = ['all-visible','numeric-only'].includes(String(w.excelSheetSelectionMode || 'all-visible')) ? String(w.excelSheetSelectionMode || 'all-visible') : 'all-visible';
    const renderedSheetSelection = ['all-visible','numeric-only'].includes(String(w.lastRenderedSheetSelectionMode || 'all-visible')) ? String(w.lastRenderedSheetSelectionMode || 'all-visible') : 'all-visible';
    if (wantedSheetSelection !== renderedSheetSelection) return false;
  }
  return true;
}
...
function showErrorPanel(title, summary, detail, actions=[]) {
  ...
  // as silence.
  // 同じ内容を何度も開き直さない。5分ごとの自動更新のたびに復活し、入力中でも
  // フォーカスとスクロールを奪っていたため、「閉じ方が分からない」「壊した」と
  // 受け取られていた。内容が変わったときだけ、改めて開く。
  const signature = `${title}||${summary}||${asArray(detail).length||String(detail||'').length}`;
  if (dismissedErrorSignature === signature) return;
  ...
  // 新しく開いたときだけ移動する。既に出ているパネルへ自動更新のたびに
  // 飛ばされると、入力中の手が止まる。
  if (wasHidden) { panel.scrollIntoView({block:'nearest'}); panel.focus({preventScroll:true}); }
}
function hideErrorPanel(dismissedByUser=false) {
  const panel = $('error-panel');
  if (panel) panel.classList.add('hidden');
  // 利用者が閉じたときだけ「この内容は見た」と覚える。状態が直って消える
  // 場合まで覚えると、次に本当に起きた失敗を出せなくなる。
  dismissedErrorSignature = dismissedByUser ? lastErrorSignature : '';
}
...
function estimateRemainingText(job, done, total) {
  const startedAt = Date.parse(String(job?.startedAt || ''));
  if (!Number.isFinite(startedAt) || done < 2 || !total || done >= total) return '';
  const elapsed = Date.now() - startedAt;
  if (elapsed <= 0) return '';
  const remainMs = (elapsed / done) * (total - done);
  const minutes = Math.ceil(remainMs / 60000);
  if (!Number.isFinite(minutes) || minutes <= 0) return '';
  return minutes <= 1 ? '・残り 1分ほど' : `・残り ${minutes}分ほど`;
}
function updateDocumentTitle(status, pct) {
  const base = 'ReportBinder';
  if (status === 'completed') document.title = `完了 - ${base}`;
  else if (status === 'completed-with-errors' || status === 'failed') document.title = `要確認 - ${base}`;
  else if (status === 'cancelled') document.title = `中止 - ${base}`;
  else document.title = `(${pct}%) PDF作成中 - ${base}`;
}
function resetDocumentTitle() { document.title = 'ReportBinder'; }
function updateProgressPanel(job) {
  ...
  // 残り時間を出す。40件で数分かかる処理に「あと何分か」が無く、会議に出るか
  // 待つかを判断できなかった。序盤は誤差が大きいので2件終わってから出す。
  const remainText = estimateRemainingText(job, done, total);
  if (countLabel) countLabel.textContent = total ? `${Math.min(done, total)} / ${total}件${remainText}` : '';
  // 会議中はタブしか見えない。タイトルに進捗を出すのがいちばん安く効く。
  updateDocumentTitle(status, pct);
...
async function scanAndRefresh(btn) {
  await runBusy(btn, async () => {
    ...
    showMessage('', '原稿の更新を確認しています', '登録済み原稿の保存日時・サイズ・内容を確認します。数十秒かかることがあります。', null, [], 0);
...
function renderAll(options={}) {
  ...
  if (!configured()) hideErrorPanel();
    ...
    showMessage('danger', 'PDF作成でエラーがあります', first?.message || `${errors}件の原稿をPDF変換できません。`, recent,
      [{label:'動作環境を診断',primary:true,handler:()=>runSystemDiagnostics($('run-diagnostics-btn'))},{label:'原稿を確認',view:'excel'}], 0);
  } else hideErrorPanel();
}
...
function renderNavBadges() {
  ...
  const agg=aggregateFinalState(); const dot=$('nav-final-dot');
  // 色の点だけでは、要対応であることも、3状態の違いも伝わらない。文字を出す。
  if(dot){const warn=['blocked','needs-rebuild','output-missing'].includes(agg.state);
    dot.classList.toggle('hidden',!warn);
    if(warn){setTextIfChanged(dot,'要対応');dot.setAttribute('title',agg.text);dot.setAttribute('aria-label',agg.text);}
    else{setTextIfChanged(dot,'');dot.removeAttribute('title');dot.removeAttribute('aria-label');}}
}
...
function renderPackProgressDashboard(){
  ...
  const stateInfo=value=>({complete:['完了','ok'],'not-started':['未着手','neutral'],'overdue-source':['期限超過','danger'],'missing-source':['必須原稿が未提出','danger'],'needs-render':['変換PDFの更新が必要','attention'],unassigned:['ページの振り分けが必要','attention'],blocked:['出力できません','danger'],'no-pages':['ページ構成が必要','attention'],'needs-output':['提出用PDFの出力が必要','attention']}[String(value)]||['対応が必要','attention']);
...
function renderFinalOverview() {
  ...
  // 5分ごとの自動更新でここが無条件に上書きされ、打ちかけの出力ファイル名が
  // 警告なく元に戻っていた。会議から戻った人は打ったつもりのまま保存を押す。
  // 触っている最中と、保存前の変更があるときは上書きしない。
  const setValue=(id,value)=>{
    const el=$(id); if(!el) return;
    const next=String(value??'');
    const dirty=el.dataset.savedValue!==undefined&&el.value!==el.dataset.savedValue;
    if(document.activeElement===el||dirty){el.dataset.savedValue=next;markUnsavedField(id);return;}
    el.value=next; el.dataset.savedValue=next; markUnsavedField(id);
  };
...
async function performPackReviewAction(action,button){
  ...
  await runBusy(button,async()=>{
    ...
    showMessage(action==='request-changes'?'warn':'ok',labels[action]||'レビュー状態を更新しました',pack.displayName||'一式');
  },false);
}
...
// 保存前の変更があることを、その場で見えるようにする。黙って元に戻すのも、
// 黙って保持するのも、どちらも「打ったはずなのに違う」を生む。
function markUnsavedField(id) {
  const el=$(id); if(!el) return;
  const dirty=el.dataset.savedValue!==undefined&&el.value!==el.dataset.savedValue;
  el.classList.toggle('has-unsaved',dirty);
  const note=document.getElementById(`${id}-unsaved`);
  if(note)note.classList.toggle('hidden',!dirty);
}
function updateBulkSelectionLabel() {
  const count = selectedPages.size;
  const el = $('bulk-selected-count');
  const visibleSelected=count?visiblePageSelectionValues().filter(id=>selectedPages.has(id)).length:0,hiddenSelected=Math.max(0,count-visibleSelected);
  if (el) el.textContent = count ? `${count}ページ選択中${hiddenSelected?`（表示外 ${hiddenSelected}）`:''}。移動先を押してください` : 'ページをクリックして選ぶと移動できます';
  // 新しいページは既定で全部「未振り分け」に入る。1枚ずつクリックさせないために、
  // 何ページ選ぶことになるのかを書いたボタンを常に見える場所へ出す。
  // (以前は閉じた折りたたみ「表示・絞り込み・並び」の中にしか無かった)
  const selectAll = $('select-all-visible-pages-btn');
  if (selectAll) {
    const visible = visiblePageSelectionValues().length;
    const remaining = visiblePageSelectionValues().filter(id => !selectedPages.has(id)).length;
    selectAll.textContent = visible ? `表示中の${visible}ページをすべて選ぶ` : '選べるページがありません';
    selectAll.disabled = pageMutationBusy || remaining === 0;
  }
  ...
  document.querySelectorAll('[data-bulk-target],#bulk-none-btn,#clear-selected-pages-btn').forEach(button=>{
    ...
    const alreadyThere=destination&&[...selectedPages].every(id=>assignedPageVolume(getPage(id))===destination);
    button.disabled=pageMutationBusy||count===0||alreadyThere;
  });
  ...
  if(undo){undo.disabled=pageMutationBusy||pageLayoutHistoryBusy||!undoEntry;undo.title=undoEntry?`${undoEntry.label}を元に戻す（Ctrl+Z）`:'元に戻せるページ構成変更はありません';undo.setAttribute('aria-label',undo.title);}
  if(redo){redo.disabled=pageMutationBusy||pageLayoutHistoryBusy||!redoEntry;redo.title=redoEntry?`${redoEntry.label}をやり直す（Ctrl+Shift+Z）`:'やり直せるページ構成変更はありません';redo.setAttribute('aria-label',redo.title);}
}
...
function updatePackTemplateDescription() {
  ...
  const description=$('pack-editor-template-description');if(description)description.textContent=String(template?.description||'Excel・Word・PowerPoint・PDFをまとめる一式です。');
}
function openPackEditor(mode='create',packId='') {
  ...
  if(mode==='rename'&&!pack){showMessage('warn','一式が見つかりません','一覧を読み込み直してください。');return;}
...
async function submitPackEditor(btn) {
  ...
  if(!name){showMessage('warn','一式名を入力してください','用途が分かる名前を入力してください。');$('pack-editor-name')?.focus();return;}
  await runBusy(btn,async()=>{
    let response;
...
async function importManagedTemplateFile(file,btn){if(!file)return;try{const body=JSON.parse(await file.text());delete body.templateId;delete body.templateVersion;const response=await api('/api/v2/pack-templates',{method:'POST',body});state.packTemplates=asArray(response.packTemplates);renderTemplateManagerSelect(String(response?.result?.templateId||''));showMessage('ok','ひな形ひな形をファイルから読み込みました','内容を確認してから一式を作成してください。');}catch(error){showMessage('danger','ひな形ひな形を読み込めません',userFriendlyError(error.message),error.detail||error.message);}finally{if(btn)btn.value='';}}
...
async function archivePack(packId) {
  const pack=findPackForManagement(packId);if(!pack)return;
  closePackMenu();
  const accepted=await confirmAction({title:'一式をアーカイブしますか？',message:`「${pack.displayName||'一式'}」を通常の一覧から隠します。`,detail:'登録情報は削除されず、アーカイブ済み一覧から復元できます。',confirmLabel:'アーカイブ',danger:true});
  if(!accepted)return;
  await runBusy(null,async()=>{await api(`/api/v2/packs/${encodeURIComponent(packId)}/archive`,{method:'POST',body:{}});await refreshPacksAfterMutation();showMessage('ok','一式をアーカイブしました','必要になったら一式一覧から復元できます。');},false);
}
async function restorePack(packId,btn) {
  const pack=findPackForManagement(packId);if(!pack)return;
  closePackMenu();
  await runBusy(btn,async()=>{await api(`/api/v2/packs/${encodeURIComponent(packId)}/restore`,{method:'POST',body:{}});await refreshPacksAfterMutation();showMessage('ok','一式を復元しました',`${pack.displayName||'一式'}を通常の一覧へ戻しました。`);},false);
}
async function deletePack(packId,btn) {
  const pack=findPackForManagement(packId);if(!pack||!pack.archived)return;
  closePackMenu();
  const accepted=await confirmAction({title:'この一式を削除しますか？',message:`「${pack.displayName||'一式'}」の登録情報とページ構成を削除します。`,detail:'元のExcel・Word・PowerPoint・PDFと、提出フォルダーへ出力済みのPDFは削除しません。ReportBinder内の原稿登録、ページ構成、出力状態は元に戻せません。',confirmLabel:'一式を削除',danger:true});
  if(!accepted)return;
  await runBusy(btn,async()=>{const response=await api(`/api/v2/packs/${encodeURIComponent(packId)}`,{method:'DELETE'});packAdminPacks=[];state=normalizeStatePayload(response.state||await api('/api/state'));await loadPackAdminPacks();renderAll();const removed=response?.result||{};showMessage('ok','一式を削除しました',`登録原稿 ${Number(removed.removedWorkbookCount||0)}件、ページ ${Number(removed.removedPageCount||0)}件を一覧から削除しました。元ファイルは残っています。`);},false);
}
async function upgradePackTemplate(packId,btn){
  ...
  try{
    ...
    if(!preview.updateAvailable){showMessage('ok','ひな形は最新です','この一式に適用できる更新はありません。');return;}
    const accepted=await confirmAction({title:'新しいひな形を適用しますか？',message:`${preview.packName||'一式'}を v${Number(preview.fromVersion||1)} から v${Number(preview.toVersion||1)} へ更新します。`,detail:[...changes.map(item=>`・${item}`),'','既存原稿とページ構成は保持され、提出用PDFは再出力が必要になります。'].join('\n'),confirmLabel:'ひな形を更新'});
    if(!accepted)return;
...
function handlePackMenuAction(event) {
  ...
  const unavailable=event.target.closest('[data-pack-unavailable]');if(unavailable){showMessage('warn','この一式は利用できません','アーカイブ済みの場合は復元してから選択してください。');return;}
  ...
  else if(action.dataset.packAction==='delete')void deletePack(packId,action);
...
async function applyPageLayoutHistory(direction,btn=null){
  if(pageLayoutHistoryBusy||pageMutationBusy)return;
  pageLayoutHistoryBusy=true;pageMutationBusy=true;updateBulkSelectionLabel();
  let button=btn;
  try{
    if(pendingPageLayoutUndo){if(boardSaveTimer){clearTimeout(boardSaveTimer);boardSaveTimer=null;}const saved=await saveBoardOrder();if(!saved)return;}
    const savedBeforeHistory=await boardSavePromise;if(!savedBeforeHistory)return;
    const redo=direction==='redo',source=redo?pageLayoutRedoStack:pageLayoutUndoStack,target=redo?pageLayoutUndoStack:pageLayoutRedoStack,entry=source[source.length-1];if(!entry)return;
    button=btn||$(redo?'page-layout-redo-btn':'page-layout-undo-btn');if(button){button.disabled=true;button.classList.add('busy');}
...
function reconcileActivePackSelection() {
  ...
  let selected = workflowPacks.find(pack => String(pack?.packId || '') === activePackId)
    ...
    // 覚えていた一式が無いときは、作成順の先頭（＝最も古い）ではなく、最後に更新された
    // ものを選ぶ。古い一式に今月の原稿を積んでしまう事故を避けるため。
    || packsByRecency(workflowPacks)[0]
    || null;
  if (!selected) {
    activePackId='';activePreset='';
    rememberActivePack('');
    try { sessionStorage.removeItem('ReportBinderCategory'); } catch {}
    return null;
  }
  activePackId = String(selected.packId || '');
  activePreset = String(selected.category||'').toLowerCase();
  rememberActivePack(activePackId);
  try { sessionStorage.setItem('ReportBinderCategory', activePreset); } catch {}
  return selected;
}
function presetLabel(preset = activePreset) {
  return String(packForPreset(preset)?.displayName || '一式未作成');
}
...
function strictNumericSheetName(name) { const raw=String(name ?? ''); return raw.length>0 && /^[0-9]+$/.test(raw) && !/[\r\n]/.test(raw); }
function numericSheetPageInfo(page) {
  const workbook=getWorkbook(page?.workbookId);
  if(sourceTypeValue(workbook)!=='excel')return {excel:false,numeric:false};
  const raw=String(page?.sheetName ?? '');
  return {excel:true,numeric:strictNumericSheetName(raw)&&page?.sheetHidden!==true,raw,canonical:raw.replace(/^0+/,'')||'0'};
}
function compareAsciiNumericText(left,right){
  if(left.length!==right.length)return left.length-right.length;
  if(left===right)return 0;
  return left<right?-1:1;
}
function compareNumericSheetPages(left,right){
  const a=numericSheetPageInfo(left),b=numericSheetPageInfo(right);
  const canonical=compareAsciiNumericText(a.canonical||'0',b.canonical||'0');
  if(canonical)return canonical;
  const af=fileOrderValue(getWorkbook(left?.workbookId)?.fileName||getWorkbook(left?.workbookId)?.displayName||left?.workbookId),bf=fileOrderValue(getWorkbook(right?.workbookId)?.fileName||getWorkbook(right?.workbookId)?.displayName||right?.workbookId);
  if(af!==bf)return af-bf;
  const file=String(getWorkbook(left?.workbookId)?.fileName||'').localeCompare(String(getWorkbook(right?.workbookId)?.fileName||''),'ja',{sensitivity:'base'});
  if(file)return file;
  const ai=Number(left?.sheetIndex),bi=Number(right?.sheetIndex);
  if(Number.isFinite(ai)&&Number.isFinite(bi)&&ai!==bi)return ai-bi;
  return resolvedPageId(left).localeCompare(resolvedPageId(right));
}
function fillExcelPageSlots(current,replacements){
  const pages=asArray(current),incoming=asArray(replacements),excelSlotIndexes=pages.map((page,index)=>({page,index})).filter(item=>numericSheetPageInfo(item.page).excel).map(item=>item.index),lastExcelSlot=excelSlotIndexes.length?excelSlotIndexes[excelSlotIndexes.length-1]:-1,result=[];
  let replacementIndex=0;
  pages.forEach((page,index)=>{
    if(numericSheetPageInfo(page).excel){
      if(replacementIndex<incoming.length)result.push(incoming[replacementIndex++]);
      if(index===lastExcelSlot)while(replacementIndex<incoming.length)result.push(incoming[replacementIndex++]);
    }else result.push(page);
  });
  if(lastExcelSlot<0)result.push(...incoming);
  return result;
}
...
// 新しく使ったものから並べる。12か月分たまると、名前だけの同じ見た目の行が
// 12本並び、どれが今月のものか分からなくなる。
function packRecencyValue(pack){
  const raw=String(pack?.updatedAt||pack?.createdAt||'');
  const t=raw?Date.parse(raw):NaN;
  return Number.isFinite(t)?t:0;
}
function packsByRecency(packs){return asArray(packs).slice().sort((a,b)=>packRecencyValue(b)-packRecencyValue(a));}
function packMenuRow(pack, archived=false) {
  ...
  const when=packRecencyValue(pack)?`・最終更新 ${formatDateTime(pack?.updatedAt||pack?.createdAt)}`:'';
  const status=(archived?'アーカイブ済み':(workflow?(active?'利用中':'選択して使用'):'現在は利用できません'))+when;
  ...
  const restoreAction=archived?`<button type="button" data-pack-action="restore" data-pack-id="${escapeAttr(id)}">復元</button><button class="danger" type="button" data-pack-action="delete" data-pack-id="${escapeAttr(id)}">削除</button>`:'';
...
function renderPackSwitcher() {
  ...
  if(button){button.disabled=!configured()&&!selected;button.classList.toggle('empty',!selected);button.setAttribute('aria-label',selected?`一式「${String(selected.displayName||'名称未設定')}」を切り替え・管理`:(configured()?'今回まとめる一式に名前を付ける':'原稿フォルダーを設定すると一式を作成できます'));button.setAttribute('aria-haspopup',selected?'dialog':'true');}
  const all=packAdminPacks.length?packAdminPacks:asArray(state?.packs);
  // 新しく使ったものを上に。作成順のままだと、毎月増えるほど今月の一式が下へ沈む。
  const active=packsByRecency(all.filter(pack=>!pack?.archived)),archived=packsByRecency(all.filter(pack=>!!pack?.archived));
  const list=$('pack-menu-list');if(list)list.innerHTML=active.length?active.map(pack=>packMenuRow(pack,false)).join(''):'<div class="pack-menu-empty">使用できる一式がありません。</div>';
...
async function togglePackMenu() {
  ...
  try{await loadPackAdminPacks();}catch(error){showMessage('danger','一式一覧を読み込めません',userFriendlyError(error.message),error.detail||error.message);}
...
function restorePageBoardScroll(snap) {
  ...
  requestAnimationFrame(() => {
    ...
    if(snap.focusPageId){const focused=[...document.querySelectorAll('.page-row')].find(row=>String(row.dataset.pageId||'')===snap.focusPageId&&!row.classList.contains('page-filter-hidden'));if(focused){syncPageRovingTabIndex(focused);focused.focus({preventScroll:true});}}
  });
}
...
function visiblePageSelectionValues() {
  // In focus mode every lane stays in the DOM so collectBoardVolumes() can
  // persist a complete order. Do not let "表示中をすべて選ぶ" select those
  // hidden lanes as well.
  return [...document.querySelectorAll('.page-row:not(.page-filter-hidden)')]
...
function syncPageSelectionUi() {
  ...
  document.querySelectorAll('.page-row').forEach(row => {
    ...
    // サムネイルは role=option、詳細は表の行。どちらもフォーカス中に
    // Spaceで変わった選択状態を支援技術へ公開する。
    if(row.matches('tr,[role="option"]'))row.setAttribute('aria-selected',String(checked));
...
async function movePageIdsToVolume(pageIds, volume, btn) {
  ...
  await runBusy(btn,async()=>{
    ...
    const activePages=pagesForActivePreset();
    for(const p of [...activePages].sort(compositionPageComparator(activePages))){const id=resolvedPageId(p);if(!id||selected.has(id))continue;const v=(p.enabled===false||String(p.volume||first)==='none')?'none':String(p.volume||first);if(!volumes[v])volumes[v]=[];volumes[v].push(id);}
...
function syncSourcePanelDensity() {
  ...
  // まだ何か残っていると誤解させるので、本当に次の手がある間だけ出す。
  const unassignedLeft=pagesForActivePreset().filter(page=>String(page?.volume||'none')==='none'&&page?.enabled!==false).length;
  const finalDone=String(aggregateFinalState().state||'')==='built';
  if(next)next.classList.toggle('hidden',registeredCount===0||needsPdf>0||(unassignedLeft===0&&finalDone));
}
function renderFileList(files) {
  ...
  box.innerHTML = `<table class="data-table file-table">
    <thead><tr><th class="check-col"><label class="check-tap"><input type="checkbox" data-select-all-files ${allSelected ? 'checked' : ''} aria-label="表示中の未登録原稿をすべて選択"></label></th><th>原稿ファイル</th></tr></thead>
...
async function applyPresetSelection(presetOrButton) {
  ...
  if(renderJobActive||activeFinalJobId){showMessage('warn','処理中は一式を切り替えられません','PDFの作成が終わってから切り替えてください。中止する場合は進捗パネルの「中止」を押してください。');return;}
  ...
  if(!pack?.workflowAvailable){showMessage('warn','この一式は利用できません','アーカイブ済みの場合は復元してから選択してください。');return;}
  ...
  if(presetChanged){
    // A queued save belongs to the currently active pack. Do not switch the
    // state container while that response can still arrive and be applied.
    if(pendingPageLayoutUndo||pendingBoardSaveVolumes){if(boardSaveTimer){clearTimeout(boardSaveTimer);boardSaveTimer=null;}await saveBoardOrder();}
    const saved=await boardSavePromise;
    if(!saved){showMessage('warn','一式を切り替えられません','ページ構成を保存できなかったため、内容を確認してからもう一度切り替えてください。');return;}
  }
  activePreset=preset;activePackId=nextPackId;rememberActivePack(activePackId);try{sessionStorage.setItem('ReportBinderCategory',activePreset);}catch{}
...
async function registerSelected(btn) {
  ...
  // 一式未作成のまま送ると、サーバーが内部互換用の識別子(ecm/bod/dmm)を含む
...
async function unregisterSelected(btn) {
  ...
  function pagesForWorkbooks(workbookIds){const set=new Set(workbookIds.map(String));return asArray(state?.pages).filter(p=>set.has(String(p?.workbookId||''))).length;}
  // 消えるのはページ構成のほうなのに、以前の文言は「変換PDFと元ファイルは削除され
  // ません」としか言わず、逆の安心を与えていた。何が失われるかを先に伝える。
  const losingPages=pagesForWorkbooks(ids);
  const accepted=await confirmAction({
    title:'原稿の登録を解除しますか？',
    message:losingPages
      ? `選択した${ids.length}件と、その${losingPages}ページの並び順・出力先・ページ名が削除されます。`
      : `選択した${ids.length}件をReportBinderの管理対象から外します。`,
    detail:losingPages
      ? 'ファイル名を変えた・移動しただけなら、「別のファイルに付け替える」を使うと、ページの並びを保ったまま差し替えられます。元の原稿ファイルと作成済みの変換PDFは削除されません。'
      : '元の原稿ファイルと作成済みの変換PDFは削除されません。',
    confirmLabel:losingPages?`登録を解除（${losingPages}ページを削除）`:'登録を解除',
    danger:true});
  if (!accepted) return;
  await runBusy(btn, async () => {
...
function renderDiffSummary(){
  ...
  const definitions=[
    ...
    ['unknown','見比べできません',Number(summary.unknown||0)],
...
function renderDiffRegionLayer(side,regions){
  ...
  // 以前は領域が13個以上のとき modified のタグを省いていた。するとその状態では
  // 「琥珀の実線枠」と「青の実線枠」を色だけで見分けることになり、加齢による
  // 青と黄の識別低下と正面衝突する。常に文字を出し、密集分は枠の内側に置く。
  const denseRegions=visible.length>12;
  for(const region of visible){
    ...
    if(y<.018||denseRegions)box.classList.add('tag-inside');
    {
...
async function saveSourceMetadata(sourceId, patch, control) {
  ...
  try{
    ...
    renderAll({preserveEditors:true});
    const savedDetail=Object.prototype.hasOwnProperty.call(patch,'excelSheetSelectionMode')?`${workbookDisplayName(getWorkbook(id))}のExcelシートPDF化対象を更新しました。次回のPDF作成から反映されます。`:`${workbookDisplayName(getWorkbook(id))}の担当情報を更新しました。`;
    showMessage('ok','原稿設定を保存しました',savedDetail,null,null,3500);
  }catch(error){
...
function sourceSettingsHtml(w){
  ...
  const sheetSelection=sourceType==='excel'?String(w?.excelSheetSelectionMode||'all-visible'):'all-visible';
  const sheetSelectionControl=sourceType==='excel'?`<label class="source-sheet-selection-label"><span>ExcelシートのPDF化対象<small>新規登録は半角数字だけの表示シートが既定です。必要なら表示中の全シートへ変更できます。</small></span><select class="source-sheet-selection" data-source-sheet-selection="${escapeAttr(id)}" aria-label="${escapeAttr(workbookDisplayName(w))}のExcelシートPDF化対象"><option value="numeric-only" ${sheetSelection==='numeric-only'?'selected':''}>半角数字だけの表示シート（既定）</option><option value="all-visible" ${sheetSelection==='all-visible'?'selected':''}>表示中の全シート</option></select></label>`:'';
  return `<details class="source-advanced-settings" data-source-settings-details="${escapeAttr(id)}" ${expandedSourceSettings.has(id)?'open':''}><summary>原稿の扱いを変更</summary><div class="source-settings" aria-label="原稿の詳細設定">${requirementSelect}${sheetSelectionControl}<label class="source-owner-label"><span>担当部署（任意）<small>進捗一覧に表示する管理用メモです</small></span><input class="source-owner-input" type="text" maxlength="120" value="${escapeAttr(owner)}" placeholder="例：営業部" data-source-owner="${escapeAttr(id)}" aria-label="${escapeAttr(workbookDisplayName(w))}の担当部署"></label><label class="source-required-label"><input type="checkbox" data-source-required="${escapeAttr(id)}" ${required?'checked':''}><span>提出用PDFに必須<small>未作成・更新あり・エラーの間は提出用PDFを出力しません</small></span></label><label class="source-default-target-label"><span>更新で増えたページの追加先<small>既に並べたページは移動しません</small></span><select class="source-default-target" data-source-default-target="${escapeAttr(id)}" aria-label="${escapeAttr(workbookDisplayName(w))}の追加ページ配置"><option value="unassigned" ${defaultTarget==='unassigned'?'selected':''}>一式の既定：${escapeHtml(activePackDefaultTargetLabel())}</option>${targetOptions}</select></label></div></details>`;
}
...
function renderWorkbooks() {
  ...
  box.querySelectorAll('[data-source-sheet-selection]').forEach(select=>select.addEventListener('change',()=>saveSourceMetadata(select.dataset.sourceSheetSelection,{excelSheetSelectionMode:select.value},select)));
...
async function renderWorkbookIds(ids, btn, options={}) {
  ...
  try{await runBusy(btn,async()=>{hideErrorPanel();showMessage('','PDF作成を開始しています',startMessage);const body={packId:activePackId};if(workbookIds.length)body.sourceIds=workbookIds;if(onlyUpdated)body.onlyUpdated=true;
    ...
    if(finished.status==='cancelled'){showMessage('warn','PDF作成を中止しました',finished.message||'未処理の原稿を残して停止しました。',finished.results||finished);setTimeout(hideProgressPanel,2400);}
      ...
      [{label:'動作環境を診断',primary:true,handler:()=>runSystemDiagnostics($('run-diagnostics-btn'))},{label:'原稿を確認',view:'excel'}], 0);
    else{const endCount=asArray(finished.results).reduce((n,r)=>n+Number(r?.sheetSync?.insertedAtEndCount||0),0);const suffix=endCount?`\n新しい${endCount}ページを末尾に追加しました。`:'';const actions=[{label:'ページ構成を確認',primary:true,view:'pages',handler:endCount?scrollToHighlightedPages:null}];if(endCount)actions.push({label:'半角数字順に並べ直す',view:'pages',handler:sortPagesByNumericSheet});else actions.push({label:'提出用PDFへ',view:'final'});showMessage(msg.kind,msg.title,msg.message+suffix,msg.detail,actions,0);setTimeout(hideProgressPanel,1800);}
...
function compareDefaultNumericPageOrder(left,right) {
  const a=numericSheetPageInfo(left),b=numericSheetPageInfo(right);
  if(a.numeric&&b.numeric){const numeric=compareNumericSheetPages(left,right);if(numeric)return numeric;}
  // Keep non-Excel/non-target pages as anchors. Numeric pages are reordered
  // against one another, while an anchor continues to follow its persisted
  // lane slot instead of being pulled to the front or back of the board.
  return pageSort(left,right);
}
function compositionPageComparator(pages=pagesForActivePreset()) {
  // The server persists the numeric default during inspection and owns the
  // explicit re-sort transaction. Rendering must always respect persisted
  // lane slots; mixing anchor and numeric comparators in Array.sort is not
...
function renderPageTargetControls(){
  ...
  const numericTarget=$('numeric-sheet-target');
  if(numericTarget){
    const previous=String(numericTarget.value||'');
    numericTarget.innerHTML=targets.map(target=>`<option value="${escapeAttr(targetVolume(target.targetId))}">${escapeHtml(target.displayName||target.targetId)}</option>`).join('');
    if(targets.length)numericTarget.value=targets.some(target=>targetVolume(target.targetId)===previous)?previous:targetVolume(targets[0].targetId);
    numericTarget.disabled=!targets.length;
    numericTarget.title=targets.length?'出力先を選択してください':'利用できる出力先がありません';
  }
}

function renderPages() {
  const box=$('page-board');if(!box)return;renderPageTargetControls();const rawPages=pagesForActivePreset();const sourcePages=[...rawPages].sort(compositionPageComparator(rawPages));const visiblePages=applyPageFilters(sourcePages);const visibleIds=new Set(visiblePages.map(resolvedPageId));const allPages=sourcePages;
  const needsPdf=workbooksForActivePreset().filter(w=>!isLatestPdfWorkbook(w));const agg=aggregateFinalState();
  const targetSignature=activeTargetDefinitions().map(target=>[target.targetId,target.displayName,target.required,target.order]);
  const signature=JSON.stringify([pageBoardView,activePageVolume,pageFilterText,showChangedOnly,[...pageVolumeCollapseOverrides.entries()],targetSignature,allPages.map(p=>{const w=getWorkbook(p.workbookId),cs=changeSummaryFor(p.workbookId);return[resolvedPageId(p),p.order,p.volume,p.enabled,p.title,pageRangeText(p),p.numberingMode,p.numberingManual,p.orderManual,p.status,p.contentPdf,p.sheetIndex,cs?.status,asArray(cs?.changedSheets),asArray(cs?.addedSheets),asArray(cs?.removedSheets),asArray(cs?.unknownSheets),asArray(p.warnings),w?.displayName,w?.fileName,w?.relativePath,w?.lastRenderedSnapshotId,w?.lastRenderedVersionId];}),needsPdf.map(w=>w.workbookId),agg.state]);
  ...
  if(needsPdf.length&&allPages.length)banners.push(`<div class="page-status-banner"><div><strong>変換PDF作成が必要な原稿があります</strong><span>${needsPdf.length}件。原稿画面でPDFを作成すると、ここでプレビューできます。</span></div><button class="btn secondary compact" type="button" data-page-go-source>原稿画面へ</button></div>`);
  if(agg.state==='needs-rebuild'&&allPages.length)banners.push('<div class="page-status-banner"><strong>ページ構成または変換PDFが変更されています</strong><span>提出用PDFの再出力が必要です。</span></div>');
  ...
  if(activePageVolume!=='all'&&!panels.some(panel=>panel.volume===activePageVolume))activePageVolume='all';
  if(activePageVolume!=='all'&&!pageVolumeAutoPicked&&allPages.length){
...
function findVolumeTabAt(x, y) {
  for (const el of document.elementsFromPoint(x, y)) {
    ...
    // "全体" changes only the view; it is not a real destination.
    if (tab && String(tab.dataset.pageVolumeTab||'')!=='all') return tab;
  }
  return null;
}
...
function handlePageRowNavigation(row,e){
  if(!['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Home','End'].includes(e.key))return false;const target=pageRowKeyboardTarget(row,e.key);if(!target)return false;e.preventDefault();syncPageRovingTabIndex(target);target.focus();target.scrollIntoView({block:'nearest',inline:'nearest'});return true;
}

function syncPageRovingTabIndex(preferred=null){
  if(pageBoardView!=='thumbnail')return;
  const rows=visiblePageRowsForKeyboard().filter(row=>row.classList.contains('page-thumb-card'));
  if(!rows.length)return;
  const focused=document.activeElement?.closest?.('.page-thumb-card');
  const target=(preferred&&rows.includes(preferred)?preferred:null)||(focused&&rows.includes(focused)?focused:null)||rows.find(row=>selectedPages.has(String(row.dataset.pageId||'')))||rows[0];
  rows.forEach(row=>row.setAttribute('tabindex',row===target?'0':'-1'));
}
...
function handleBoardRowKeydown(row,e){
  ...
  // Navigation/selection remain available while saving, but a second layout
  // mutation must not start from the keyboard with the same baseLayout.
...
function delegateBoardEvents(box){
  ...
  box.addEventListener('click',e=>{
    if(e.target.closest('[data-page-go-source]')){setActiveView('excel');return;}
    if(e.target.closest('[data-clear-page-filters]')){pageFilterText='';showChangedOnly=false;const input=$('page-search-input'),toggle=$('changed-only-toggle');if(input)input.value='';if(toggle)toggle.checked=false;$('page-search-clear')?.classList.add('hidden');lastPageBoardRenderSignature='';renderPages();return;}
  ...
  box.addEventListener('keydown',e=>{
    ...
    if(editor){
      if(e.key==='Escape'){e.preventDefault();const card=editor.closest('.page-thumb-card');setPageThumbnailEditor(card,false,true);syncPageRovingTabIndex(card);card?.focus();return;}
...
function attachBoardEvents() {
  ...
  box.querySelectorAll('.page-row').forEach(row=>row.setAttribute('aria-keyshortcuts',`${row.classList.contains('page-thumb-card')?'F2 ':''}Space Enter ArrowUp ArrowDown ArrowLeft ArrowRight Home End Alt+ArrowUp Alt+ArrowDown Alt+ArrowLeft Alt+ArrowRight Control+A`));
...
function rememberPageMutationFingerprint(payload,packId=null){
  const result=payload?.result||payload||{};
  if(result.layoutFingerprint)rememberLayoutFingerprint(String(packId||activePackRecord()?.packId||activePackId),result.layoutFingerprint);
}
...
function saveBoardOrder() {
  ...
  // Freeze the pack identity when queued, but read its fingerprint only when
  // this serialized request starts. That lets save B use save A's new token.
  const requestPackId=String(activePackRecord()?.packId||activePackId||'');
  const persist=async()=>{
    try{
      const requestBody={packId:requestPackId,volumes};const base=activeLayoutFingerprint(requestPackId);if(base)requestBody.baseLayout=base;
      const response=await api('/api/v2/items/reorder',{method:'POST',body:requestBody});
      const newerBoardExists=requestRevision!==boardSaveRevision;
      // Even when a newer local layout is waiting, its request must be based on
      // this successful save. Suppress only the older DOM payload, never the
      // server's new optimistic-lock fingerprint.
      rememberPageMutationFingerprint(response,requestPackId);
      if(!newerBoardExists){applyPageMutationResult(response);setPageSaveStatus('saved','保存済み');}
...
async function collectNumericSheets(btn=null){
  ...
  const requestPackId=String(activePackRecord()?.packId||activePackId||''),requestBaseLayout=activeLayoutFingerprint(requestPackId);
  const activePages=pagesForActivePreset(),pages=[...activePages].sort(compositionPageComparator(activePages)),volumeKeys=[...targets,'none'],byVolume=Object.fromEntries(volumeKeys.map(key=>[key,[]]));
  for(const page of pages){const volume=assignedPageVolume(page);if(!byVolume[volume])byVolume[volume]=[];byVolume[volume].push(page);}
  const numeric=[],excluded=[];
  for(const page of pages){const info=numericSheetPageInfo(page);if(!info.excel)continue;if(info.numeric)numeric.push(page);else excluded.push(page);}
  if(!numeric.length){showMessage('warn','半角数字シートがありません','表示中のExcelページに、シート名が半角数字だけ（例：1、2、10）のものがありません。必要なシートを表示し、シート名を変更してからもう一度実行してください。');return;}
  numeric.sort(compareNumericSheetPages);
  const nonExcelPages=page=>!numericSheetPageInfo(page).excel;
  const desired=Object.fromEntries(volumeKeys.map(key=>[key,[]]));
  desired[target]=fillExcelPageSlots(byVolume[target]||[],numeric);
  for(const key of volumeKeys){
    if(key===target)continue;
    desired[key]=(byVolume[key]||[]).filter(nonExcelPages);
  }
  // Every excluded Excel page, including an existing hidden-sheet record, is
  // deliberately made unassigned. Numeric candidates are already represented
  // once in the selected target, so they are not copied into none.
  desired.none=fillExcelPageSlots(byVolume.none||[],excluded);
  const before=collectBoardVolumes();
  const normalizedBefore=Object.fromEntries(volumeKeys.map(key=>[key,asArray(before[key])]));
  const normalizedAfter=Object.fromEntries(volumeKeys.map(key=>[key,asArray(desired[key]).map(resolvedPageId)]));
  if(pageVolumeSnapshotsEqual(normalizedBefore,normalizedAfter)){
    showMessage('ok','すでに半角数字シートの配置です','数字順の並びと、その他のページの配置に変更はありません。');
    return;
  }
  const targetLabel=assignmentVolumeLabel(target),excludedLabel=excluded.length?`半角数字以外または非表示のExcel ${excluded.length}ページは未振り分けへ戻します。`:'';
  const accepted=await confirmAction({
    title:'半角数字シートをまとめますか？',
    message:`表示中のExcelページ ${numeric.length}件を「${targetLabel}」へ数字順で配置します。`,
    detail:[excludedLabel,'Excel以外のページは現在の配置と順番を保持します。これは今回のページ構成に対する一度の操作です。再取込後は必要に応じてもう一度実行してください。','実行後は元に戻せます。'].filter(Boolean).join('\n'),
    confirmLabel:'まとめる'
  });
  if(!accepted)return;
...
async function sortPagesByNumericSheet(btn=null) {
  if(pageMutationBusy||pageLayoutHistoryBusy)return;
  pageMutationBusy=true;updateBulkSelectionLabel();
  try{
    // A drag save may already be queued. Sort only after it advances the
    // optimistic-lock fingerprint, otherwise both requests share the same
    // baseLayout and one necessarily loses.
    const priorSaveSucceeded=await boardSavePromise;
    if(!priorSaveSucceeded)return;
    const beforeVolumes=collectBoardVolumes(),requestRevision=boardSaveRevision;
    const numericBefore=numericSheetSequenceIsDefault();
    // The server treats a sequence-equal lane as a true no-op. Mirror that
    // contract here so stale orderManual metadata cannot create a request,
    // history entry, or misleading "sorted" notification without a visible
    // layout change.
    if(numericBefore){
      showMessage('ok','すでに半角数字順です','各レーンの半角数字シートは数字順に並んでいます。');
      return;
    }
    setPageSaveStatus('saving','保存中…');
    await runBusy(btn||$('sort-by-sheet-btn'),async()=>{
      try{
        const response=await api('/api/pages/sort-by-numeric-sheet',{method:'POST',body:pageApiBody({volumes:[...activeTargetVolumes(),'none']})});
        applyPageMutationResult(response);setPageSaveStatus('saved','保存済み');
        const changed=!pageVolumeSnapshotsEqual(beforeVolumes,collectBoardVolumes());
        if(changed)rememberPageLayoutUndo('半角数字順への整列',beforeVolumes);
        showMessage('ok',changed?'半角数字順に並べ直しました':'すでに半角数字順です',changed?'未振り分け・本体・補足の各レーンを半角数字シート順に整列しました。':'ページの並びに変更はありませんでした。',null,changed?[{label:'元に戻す',handler:()=>undoLastPageLayout()},{label:'提出用PDFへ',view:'final'}]:[{label:'提出用PDFへ',view:'final'}],8000);
      }catch(error){
        setPageSaveStatus('error','保存できません');
        if(error.code==='structure-conflict'){await handlePageLayoutConflict(beforeVolumes,requestRevision);return;}
        throw error;
      }
    });
...
async function sortPagesBySheet(btn=null){return sortPagesByNumericSheet(btn);}
...
function previewOrganizerPageIds(pageId){
  const current=getPage(pageId);if(!current)return[];const volume=assignedPageVolume(current),visible=visiblePageSelectionValues(),useVisible=visible.includes(String(pageId)),activePages=pagesForActivePreset(),ordered=useVisible?visible.map(getPage).filter(Boolean):[...activePages].sort(compositionPageComparator(activePages));return ordered.filter(page=>assignedPageVolume(page)===volume&&pagePreviewAvailable(page)).map(resolvedPageId);
}
...
async function savePaths(btn) {
  ...
  if (!submissionDir) {
    showMessage('warn', '原稿フォルダーを入力してください', 'フォルダーの場所を入力するか、「フォルダーを選ぶ」を使用してください。');
    $('submissionDir')?.focus();
    return;
  }
...
async function runBusy(btn, fn, showProcessing=true) {
  ...
  if(showProcessing)showMessage('','処理中です','完了すると画面が更新されます。',null,[],0);
  // ほぼ全ての操作の失敗がここに落ちる。以前はボタンが1つも無く、
  // 「押してはいけないものを押したのか」と考えて手が止まっていた。
  // 少なくとも「もう一度試す」と、原因を調べる入口は常に出す。
  try{await fn();}catch(e){
    hideProgressPanel();
    const actions=[{label:'もう一度試す',primary:true,handler:()=>runBusy((btnId&&$(btnId))||btn,fn,showProcessing)},
                   {label:'動作環境を診断',handler:()=>runSystemDiagnostics($('run-diagnostics-btn'))}];
    showMessage('danger','処理できませんでした',userFriendlyError(e.message),e.detail||e.stack||e.message,actions,0);
  }
  finally{
...
$('pack-output-pattern')?.addEventListener('input',()=>markUnsavedField('pack-output-pattern'));
...
bind('notice-close','click',hideMessage);bind('error-close','click',()=>hideErrorPanel(true));bind('scan-btn','click',()=>scanAndRefresh($('scan-btn')));
...
bind('clear-selected-files-btn','click',clearVisibleFilesSelection);bind('clear-selected-workbooks-btn','click',clearWorkbookSelection);bind('select-all-visible-pages-btn','click',selectAllActivePages);bind('clear-selected-pages-btn','click',clearActivePageSelection);
...
bind('register-selected-btn','click',()=>registerSelected($('register-selected-btn')));bind('unregister-selected-btn','click',()=>unregisterSelected($('unregister-selected-btn')));bind('render-selected-btn','click',()=>renderSelectedWorkbooks($('render-selected-btn')));bind('render-all-btn','click',()=>renderAllWorkbooks($('render-all-btn')));bind('sort-by-sheet-btn','click',()=>sortPagesByNumericSheet($('sort-by-sheet-btn')));bind('collect-numeric-sheets-btn','click',()=>collectNumericSheets($('collect-numeric-sheets-btn')));
...
async function loadInitialAppState(button=null) {
  ...
  try {
    ...
    if(message)message.textContent=SERVER_GONE_MESSAGE;
...
function renderAutoStatus(){
  ...
  const lines=items.slice(0,8).map(it=>{
    ...
    const label={idle:it.lastResult==='completed'?'完了':'待機中',waiting:'変更を検知（保存が落ち着くまで待つ時間）',deferred:'保留中',rendering:'PDF作成中',ready:'実行待ち','retry-wait':'再試行待ち',failed:'自動処理失敗'}[String(it.state||'')]||String(it.state||'');
...
async function loadLayoutSnapshots(){
  ...
  try{
    ...
    const reasonLabel={reorder:'ページの移動・並べ替え','sort-by-sheet':'半角数字順に並べ直す','sort-by-numeric-sheet':'半角数字順に並べ直す','page-update':'ページ設定の変更','volume-change':'出力先の変更',numbering:'ページ番号の変更',enabled:'出力対象の変更',restore:'復元',
      'pre-restore':'復元の直前'};
...
async function previewLayoutRestore(snapshotId, btn){
  ...
  }catch(e){if(restoreStarted)setPageSaveStatus('error','保存できません');showMessage('danger','復元できません',userFriendlyError(e.message));}
...
async function loadFinalArchives(){
  ...
  if(!box)return;
  if(!activePackRecord()?.packId){box.innerHTML='<div class="caption">一式を作成すると出力履歴が表示されます。</div>';return;}
  try{
